...
def ajustar_archivo_afo(data,config):
    '''
    ...
    data['modelo_atencion'] = data[config['lista_columnas_afo'][0]].astype(str).str.startswith('8').map({True: 'Indirecta', False: 'Directa'})
    # realizando tipado de las columnas de mes y reemplazos de valores 0 a nulos
       
    data[config['listo_meses_completos']] = data[config['listo_meses_completos']].astype(float)
    data[config['listo_meses_completos']] = data[config['listo_meses_completos']].replace(0, np.nan)
    #contando valores no nulos en las columnas.
    data['conteo_meses'] = data[config['listo_meses_completos']].notna().sum(axis=1)
    data['ultimo_mes_con_ventas'] = data.apply(obtener_ultimo_mes,axis=1,args=(config['listo_meses_completos'],))
    data['ventas_acumuladas'] = data[config['listo_meses_completos']].sum(axis=1, skipna=True)
    ...
    data['total_meses_analizado'] = len(config['listo_meses_completos'])
